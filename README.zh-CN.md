...
Koma 会把音频发送给已配置的听写服务，并把关键帧与字幕上下文发送给已配置的视觉模型。原视频、关键帧、结果和生成文件会一直保存，直到提交任务的浏览器或管理员删除；拿到不可猜回看链接的人都可以查看结果。清除站点数据或更换浏览器后，将失去原浏览器“我的任务”里的管理入口。完整边界见[管理平台](docs/ADMIN.zh-CN.md)。
