...
`url` 必须是路径以 `.mp4` 结尾的 HTTP(S) 文件直链（后缀不区分大小写），支持签名查询参数。平台页面、分享链接或文案、播放列表、无后缀接口、包含用户名或密码的链接以及本机或内网 IP 字面地址，会在创建任务或消耗演示额度之前返回 `400`。不会进行平台解析或调用解析器兜底。
...
视频地址提交需要 GitHub 登录。Koma 会校验初始 URL 和每次重定向，但不会固定 DNS，尚未完整阻止解析到私有、链路本地地址的域名。身份验证只能限制调用者，不能充当出站网络隔离；允许不可信用户使用的部署仍应配置出站策略或可信 URL 白名单。

`202` 只表示输入已受理，不保证远程文件可用。下载、响应与文件类型校验、大小和时长限制、本地视频检查在任务内完成；失败会在 AI 分析之前写入任务状态。重定向目标也必须是 MP4 直链。
...
只有失败且保留有效 MP4 直链或视频文件的任务可以重试。仅保留旧平台链接的任务不能重试；已经保存的视频文件仍可复用。重试复制原语言和分析要求，使用当前配置的 Provider，并保留原失败记录；有保存的视频时优先使用，不重新下载 URL。任务正在执行或已完成、来源缺失、保留视频无法读取时返回 `409`；其他账号的任务返回 `404`。重试同样受分析权限和单 IP 演示额度限制；保存的 URL 仍可能在新分析中因过期或站点问题而失败。
