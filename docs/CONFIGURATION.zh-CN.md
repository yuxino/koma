...
1. 校验并下载用户提供的 MP4 直链，或接收本地上传的视频。
2. 使用 FFmpeg 抽取代表性画面。
...
本地上传保留现有的视频格式支持。远程输入只接受路径以 `.mp4` 结尾的 HTTP(S) 文件直链（后缀不区分大小写），保留签名查询参数。不支持无文件后缀的接口地址、平台页面、分享链接或文案、播放列表。

Koma 只下载用户提供的文件，不调用平台接口、不解析网页、不注册 Cookie、不模拟浏览器身份，也没有解析器兜底。每次重定向的目标仍须是 MP4 直链。分析前会检查响应类型、MP4 文件头、文件大小和本地视频元数据；通用二进制响应也必须通过这些检查。返回网页时直接拒绝，不继续寻找视频地址。

不接受包含用户名或密码的链接，也不访问本机或内网 IP 字面地址；不支持需要登录或订阅权限的链接。校验不包含 DNS 固定，公开部署仍需出站网络策略或可信 URL 白名单，防止域名解析到内网。
