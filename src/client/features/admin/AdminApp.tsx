...
export default function AdminApp() {
  ...
  return <div className="admin-shell">
    ...
    {session === null ? <main className="admin-centered"><div className="admin-loader" /></main>
      ...
      : !session.authenticated ? <main className="admin-centered"><form className="admin-login" onSubmit={login}><img src="/koma-ponytail-portrait.webp" alt="" /><span>{t.console}</span><h1>{t.loginTitle}</h1><p>{t.loginText}</p><label><strong>{t.password}</strong><input type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" autoFocus required /></label><button className="primary-button" type="submit" disabled={busy}>{busy ? t.signingIn : t.signIn}</button>{error && <p className="admin-error" role="alert">{error}</p>}</form></main>
  ...
  return <div className="modal-backdrop admin-detail-backdrop" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <article className="job-detail-modal" role="dialog" aria-modal="true" aria-labelledby="job-detail-title">
      <header className="job-detail-header">
        ...
        <div className="job-detail-header-actions">{detail && <a className="secondary-button" href={`/jobs/${detail.id}`} target="_blank" rel="noreferrer">{t.fullReplay}</a>}<button className="modal-close" type="button" onClick={onClose} aria-label={t.closeDetails}><Icon name="close" size={18} /></button></div>
      </header>
          ...
          : detail && <>
            ...
            <section className="job-detail-section">
              ...
              {result ? <>
                ...
                {result.artifacts?.length ? <DetailBlock label={t.generatedFiles}><div className="job-artifact-list">{result.artifacts.map((artifact) => artifact.downloadUrl ? <a key={artifact.id} href={artifact.downloadUrl}><span><strong>{artifact.name}</strong><small>{artifact.format.toUpperCase()} · {formatBytes(artifact.sizeBytes)}</small></span><Icon name="download" size={18} /></a> : <span key={artifact.id}>{artifact.name}</span>)}</div></DetailBlock> : null}
...
function AdminMessage({ title, text }: { title: string; text: string }) {
  return <div className="admin-message"><div className="admin-message-icon"><Icon name="close" size={24} /></div><h1>{title}</h1><p>{text}</p><a className="primary-button" href="/">Koma</a></div>;
}
