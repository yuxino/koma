...
export default function AdminApp() {
  ...
  return <div className="admin-shell">
    ...
    {session === null ? <main className="admin-centered"><div className="admin-loader" /></main>
      ...
      : !session.authenticated ? <main className="admin-centered"><form className="admin-login" onSubmit={login}><img src="/koma-companion-girl.png" alt="" /><span>{t.console}</span><h1>{t.loginTitle}</h1><p>{t.loginText}</p><label><strong>{t.password}</strong><input type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" autoFocus required /></label><button className="primary-button" type="submit" disabled={busy}>{busy ? t.signingIn : t.signIn}</button>{error && <p className="admin-error" role="alert">{error}</p>}</form></main>
