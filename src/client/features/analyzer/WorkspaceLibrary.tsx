...
export function WorkspaceLibrary({ language, onOpen, onDelete, onNew, onUnauthorized, legacyJobCount, onClaimed }: {
  ...
  return <section className="workspace-library" aria-labelledby="library-title">
    <header className="library-heading"><div><span className="page-label">YOUR FILM NOTES</span><h1 id="library-title">{t.title}</h1><p>{t.description}</p></div><button className="primary-button" type="button" onClick={onNew}><Icon name="plus" size={20} />{t.create}</button></header>
