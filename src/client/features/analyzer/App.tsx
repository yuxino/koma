...
const copy = {
  en: {
    ...
    hero: "Summarize a video and search its subtitles.",
    intro: "Upload a video or paste a direct MP4 URL. Review the summary and chapters, search the subtitles, or extract custom data fields.",
    mascotCue: "Ready to analyze your video.",
    mascotCueText: "I use speech and key frames to build a summary you can check against the video.",
    flowVideo: "VIDEO",
    ...
    temporary: "Saved until you or an administrator deletes it",
    ...
    progressTitle: "Generating the video analysis.",
    progressText: "Processing speech and key frames. When the analysis is ready, click a result to jump to the matching moment.",
  ...
  zh: {
    ...
    badge: "视频分析与数据提取",
    hero: "整理视频摘要，搜索字幕。",
    intro: "上传视频或粘贴 MP4 直链。查看摘要和章节、搜索字幕，或提取你指定的数据字段。",
    mascotCue: "准备好分析视频了。",
    mascotCueText: "我会结合语音和关键帧整理摘要，你可以对照原片核实。",
    flowVideo: "视频",
    flowSignals: "声音 + 画面",
    flowOutput: "结构化 JSON",
    newAnalysis: "NEW ANALYSIS",
    ...
    analyzingRemote: "ANALYZING · REMOTE VIDEO",
    analyzingLocal: "ANALYZING · LOCAL VIDEO",
    progressTitle: "正在生成视频分析结果。",
    progressText: "正在处理语音和关键帧。完成后可点击结果，回到原片对应时刻。",
    processing: "处理中",
    preparing: "正在准备…",
    entered: "视频已接收",
...
function ResultView({ job, onRestart, onDelete, language }: { job: Job; onRestart: () => void; onDelete: (id: string) => Promise<boolean>; language: Language }) {
  ...
  return <section className="result-layout"><div className="result-main">
    ...
    <section className="tag-panel"><div className="section-heading"><span>{t.contentTags}</span><small>{t.jumpTag}</small></div><div className="tag-list">{(result.tags || []).map((tag) => <button type="button" className="tag-chip" key={`${tag.category}-${tag.label}`} onClick={() => syncToTime(tag.atMs)}><span>{translateTagCategory(tag.category, language)}</span>{tag.label}<i>{formatTime(tag.atMs)}</i></button>)}</div></section>
