...
export function WelcomeScreen({ language, enabled, unavailable, expired, error, onRefresh }: {
  ...
  return <section className="welcome-layout" aria-labelledby="welcome-title">
    <div className="welcome-main"><span className="welcome-eyebrow">{zh ? "AI 视频分析" : "AI VIDEO ANALYSIS"}</span><h1 id="welcome-title">{zh ? <>整理视频摘要，<br /><span>随时回看原片。</span></> : <>Summarize a video.<br /><span>Find a moment.</span></>}</h1><p className="welcome-description">{zh ? "上传课程、访谈或演示视频，或粘贴 MP4 直链。Koma 生成摘要、章节和可搜索字幕；点击结果，就能回到原片对应时刻。" : "Upload a lesson, interview, or demo, or paste a direct MP4 URL. Koma generates a summary, chapters, and searchable subtitles. Click a result to return to that moment in the video."}</p>
      ...
      <div className="welcome-account-note"><Icon name="corner-down-right" size={21} /><p>{zh ? "视频和分析记录保存在你的账号下。换个浏览器，用同一个 GitHub 账号登录即可查看。" : "Your videos and analysis history are saved to your account. Sign in with the same GitHub account to access them in another browser."}</p></div>
    </div>
    <aside className="welcome-aside"><div className="welcome-character"><span className="character-greeting">{zh ? "今天，一起看点什么？" : "What shall we watch today?"}</span><CompanionPortrait priority /><span className="character-signature">{zh ? "Koma，陪你查看画面和字幕。" : "Koma, here to help with frames and subtitles."}</span></div></aside>
    <div className="welcome-capabilities">{(zh ? [["01", "先看摘要和章节", "摘要与章节，帮助你快速了解内容和脉络。"], ["02", "搜索字幕，回到原片", "搜索字幕，点一下就回到原片中的那一刻。"], ["03", "下载分析结果", "摘要、字幕、自定义数据，下载后继续使用。"]] : [["01", "Summaries and chapters", "Read a summary and browse the video by chapter."], ["02", "Search and replay", "Search a line and jump straight to where it happened."], ["03", "Download the results", "Download your summary, subtitles, and custom data."]]).map(([index, title, description]) => <div key={index}><span>{index}</span><div><h2>{title}</h2><p>{description}</p></div></div>)}</div>
    <div className="welcome-details">
      <section><h2>{zh ? "在浏览器里分析视频" : "Video analysis in your browser"}</h2><p>{zh ? "Koma 是可自行部署的 AI 视频分析网站。通过 GitHub 登录后，上传本地视频或粘贴 MP4 视频直链，把语音和关键帧整理成摘要、章节、字幕与自定义数据。使用在线工作区需要 JavaScript，无需安装桌面应用。" : "Koma is a self-hosted AI video analysis website. Sign in with GitHub, upload a video or paste a direct MP4 URL, and turn speech and key frames into summaries, chapters, subtitles, and custom data. The online workspace needs JavaScript; no desktop app is required."}</p></section>
      ...
      <section><h2>{zh ? "自行部署与使用限制" : "Self-hosting and limits"}</h2><p>{zh ? "自行部署需要 Node.js 22.13 或更新版本，并配置 GitHub 登录、语音与视觉服务；未配置模型服务时使用模拟输出。上传默认限 500 MB、15 分钟，实际以站点配置为准。只接受本地上传或可直接下载的 MP4 直链，不解析第三方平台页面、分享链接或需要登录的视频。" : "Self-hosting requires Node.js 22.13 or newer, GitHub sign-in, and speech and vision providers. Unconfigured providers produce mock output. Uploads default to 500 MB and 15 minutes, subject to the site's settings. Only local uploads and downloadable MP4 URLs are accepted. Third-party platform pages, share links, and login-protected media are not supported."}</p><a href={`${REPOSITORY_URL}/blob/main/${zh ? "README.zh-CN.md" : "README.md"}`}>{zh ? "源码与部署指南" : "Source and setup guide"}</a></section>
    </div>
    <footer className="welcome-footer"><span>KOMA · AI VIDEO ANALYSIS</span><span>{zh ? "视频理解 · 个人资料库" : "VIDEO ANALYSIS · YOUR OWN LIBRARY"}</span></footer>
  </section>;
}
