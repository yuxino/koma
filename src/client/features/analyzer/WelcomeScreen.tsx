...
export function WelcomeScreen({ language, enabled, unavailable, expired, error, onRefresh }: {
  ...
  return <section className="welcome-layout" aria-labelledby="welcome-title">
    <div className="welcome-main"><span className="welcome-eyebrow">{zh ? "你的视频小助手" : "YOUR VIDEO COMPANION"}</span><h1 id="welcome-title">{zh ? <>看视频，也能<br /><span>轻松做笔记。</span></> : <>A little help with<br /><span>your next video.</span></>}</h1><p className="welcome-description">{zh ? "想学的课程、喜欢的访谈、来不及看完的视频。交给 Koma，把重点整理成笔记，想重看的那一句，也能马上找到。" : "The lesson you’re learning, the interview you loved, the video you haven’t finished. Koma gathers the useful parts into notes, with every moment easy to find again."}</p>
