...
const media = vi.hoisted(() => ({ inspectVideo: vi.fn(), downloadUrl: vi.fn(), prepareDirectVideoUrl: vi.fn() }));
...
vi.mock("../media/url-source.js", () => ({ prepareDirectVideoUrl: media.prepareDirectVideoUrl }));
let root: string;
beforeEach(async () => {
  ...
  vi.resetModules();
  media.prepareDirectVideoUrl.mockResolvedValue("https://example.com/public-fixture.mp4");
});
...
describe("pipeline storage failure recovery", () => {
  ...
  it("ignores URL validation that arrives after its job was deleted", async () => {
    const jobs = await import("./jobs.js");
    const release = vi.spyOn(jobs, "releaseWorkingDirectory");
    const database = await import("../persistence/database.js");
    const resolved = Promise.withResolvers<string>();
    media.prepareDirectVideoUrl.mockReturnValueOnce(resolved.promise);
    const job = await jobs.createJob({ source: "url", title: "deleted", sourceUrl: "https://example.com/fixture.mp4" });
    (await import("./pipeline.js")).enqueueAnalysis(job);
    await vi.waitFor(() => expect(media.prepareDirectVideoUrl).toHaveBeenCalledOnce());
    await jobs.deleteJob(job.id);
    resolved.resolve("https://example.com/video.mp4");
  ...
  it.each(["during deletion", "after deletion fails"])("preserves recovery files when canceled analysis finishes %s", async (timing) => {
    ...
    const job = await jobs.createJob({ source: "url", title: "recover", sourceUrl: "https://example.com/fixture.mp4" });
